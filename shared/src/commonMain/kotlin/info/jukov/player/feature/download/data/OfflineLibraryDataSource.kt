...
internal fun observeOfflineLibrary(
    dao: CacheDao,
    platform: OfflinePlatform,
    client: SubsonicApiClient,
    session: AuthSession,
...
internal fun observeDownloadStatuses(
    dao: CacheDao,
    accountKey: String,
): Flow<Map<String, DownloadStatus>> = dao.observeOfflineTracks(accountKey).map { tracks ->
    tracks.associate { it.trackId to it.toDownloadStatus() }
}

internal fun observeDownloadFailureSummary(
    dao: CacheDao,
    accountKey: String,
): Flow<DownloadFailureSummary> = dao.observeOfflineTracks(accountKey).map { it.toFailureSummary() }
